...
void test_insert_descending_order() {
    ...
    my_list.insert(2);
    my_list.insert(1);
    assert(my_list.to_string() == "[1, 2, 3, 4, 5]");
    cout << "Insert descending order test passed.\n";
}

void test_insert_identical_values() {
    list my_list;
    my_list.insert(2);
    my_list.insert(3);
    my_list.insert(3);
    my_list.insert(3);
    my_list.insert(4);
    assert(my_list.to_string() == "[2, 3, 3, 3, 4]");
    cout << "Insert identical values test passed.\n";
}
...
void test_large_number_of_inserts() {
    ...
    // Verify the first and last elements as a simple check for correct ordering
    assert(my_list.to_string().substr(1, 3) == "1, "); // Checks the start
    assert(my_list.to_string().substr(my_list.to_string().size() - 3) == ", 100]"); // Checks the end
    cout << "Large number of inserts test passed.\n";
}
